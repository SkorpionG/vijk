...
// Triangle vertices
const vertex1: Vector = new Vector([0, 0, 0]);
const vertex2: Vector = new Vector([1, 0, 0]);
const vertex3: Vector = new Vector([0, 1, 0]);

// Edge vectors
const edge1: Vector = Vector.fromPoints(vertex1, vertex2);
const edge2: Vector = Vector.fromPoints(vertex1, vertex3);

// Surface normal (cross product)
const surfaceNormal: Vector = edge1.cross(edge2).normalize();
...
const cameraPosition: Vector = new Vector([0, 0, 10]);
const targetPosition: Vector = new Vector([0, 0, 0]);

// View direction (from camera to target)
const viewDirection: Vector = Vector.fromPoints(cameraPosition, targetPosition).normalize();

// Up vector
const worldUp: Vector = new Vector([0, 1, 0]);

// Right vector (cross product of view and up)
const rightVector: Vector = viewDirection.cross(worldUp).normalize();

// Camera up (cross product of right and view)
const cameraUp: Vector = rightVector.cross(viewDirection).normalize();
...
const lightDirection: Vector = new Vector([1, 1, 1]).normalize();
const normal: Vector = new Vector([0, 0, 1]); // Surface facing camera

// Diffuse lighting intensity (dot product)
const diffuseIntensity: number = Math.max(0, normal.dot(lightDirection));
...
const incident: Vector = new Vector([1, -1, 0]).normalize();
const surfaceNorm: Vector = new Vector([0, 1, 0]);

// Reflected ray: R = I - 2(I·N)N
const dotProduct: number = incident.dot(surfaceNorm);
const reflected: Vector = incident.subtract(surfaceNorm.scale(2 * dotProduct));
...
const planePoint: Vector = new Vector([0, 0, 0]);
const planeNormal: Vector = new Vector([0, 1, 0]); // XZ plane

// Point in space
const point: Vector = new Vector([5, 3, 2]);

// Distance = |(P - P0) · N|
const vectorToPoint: Vector = Vector.fromPoints(planePoint, point);
const distance: number = Math.abs(vectorToPoint.dot(planeNormal));
...
// Triangle normal
const triangleNormal: Vector = new Vector([0, 0, 1]);

// View direction from camera
const cameraToTriangle: Vector = new Vector([0, 0, -1]);

// If dot product is negative, triangle faces away from camera
const facingCamera: number = triangleNormal.dot(cameraToTriangle);
const isVisible: boolean = facingCamera < 0;
...
const object1Center: Vector = new Vector([0, 0, 0]);
const object1HalfExtents: Vector = new Vector([1, 1, 1]);

const object2Center: Vector = new Vector([1.5, 0, 0]);
const object2HalfExtents: Vector = new Vector([1, 1, 1]);

// Distance between centers
const centerDistance: Vector = Vector.fromPoints(object1Center, object2Center);
const distanceMagnitude: number = centerDistance.magnitude();

// Sum of half-extents
const sumHalfExtents: number = object1HalfExtents.get(0) + object2HalfExtents.get(0);

const isColliding: boolean = distanceMagnitude < sumHalfExtents;
...
const originalPoint: Vector = new Vector([1, 0, 0]);
const rotationAngle: number = Math.PI / 2; // 90 degrees

// Rotation matrix for Y-axis
const rotatedX: number =
  originalPoint.get(0) * Math.cos(rotationAngle) + originalPoint.get(2) * Math.sin(rotationAngle);
const rotatedY: number = originalPoint.get(1);
const rotatedZ: number =
  -originalPoint.get(0) * Math.sin(rotationAngle) + originalPoint.get(2) * Math.cos(rotationAngle);

const rotatedPoint: Vector = new Vector([rotatedX, rotatedY, rotatedZ]);
...
const startPos: Vector = new Vector([0, 0, 0]);
const endPos: Vector = new Vector([10, 5, 3]);
const t: number = 0.5; // 50% between start and end

// Lerp: P = (1-t)A + tB = A + t(B-A)
const interpolated: Vector = startPos.add(Vector.fromPoints(startPos, endPos).scale(t));
...
// Ray
const rayOrigin: Vector = new Vector([0, 0, -5]);
const rayDir: Vector = new Vector([0, 0, 1]);

// Sphere
const sphereCenter: Vector = new Vector([0, 0, 0]);
const sphereRadius: number = 1;

// Vector from ray origin to sphere center
const oc: Vector = Vector.fromPoints(sphereCenter, rayOrigin);

// Quadratic equation coefficients
const a: number = rayDir.dot(rayDir);
const b: number = 2.0 * oc.dot(rayDir);
const c: number = oc.dot(oc) - sphereRadius * sphereRadius;

const discriminant: number = b * b - 4 * a * c;
const hits: boolean = discriminant >= 0;
...
if (hits) {
  const t1: number = (-b - Math.sqrt(discriminant)) / (2.0 * a);
  const hitPoint: Vector = rayOrigin.add(rayDir.scale(t1));
